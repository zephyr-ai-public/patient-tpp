...
import numpy as np
from easy_tpp.preprocess.event_tokenizer import BatchEncoding, EventTokenizer
...
class ZaiEventTokenizer(EventTokenizer):
    """
    ...
    def __init__(self, config: ZaiConfig, use_pad_token_id: Optional[int] = None):
        config = copy.deepcopy(config)
        self.num_event_types = config.num_event_types
        self.pad_token_id = (
            config.pad_token_id if use_pad_token_id is None else use_pad_token_id
        )
        ...
        self.use_invariant_features = config.use_invariant_features
    ...
    def _get_padding_truncation_strategies(
        ...
        verbose: bool = False,
        **kwargs: Any,
    ) -> Tuple[PaddingStrategy, TruncationStrategy, Optional[int], dict[str, Any]]:
    ...
    def _truncate(
    ...
    ) -> Union[Dict[str, Any], Dict[str, List]]:
        if max_length is None:
            max_length = 64
        ...
        if truncation_strategy != TruncationStrategy.DO_NOT_TRUNCATE:
            ...
            for k, v in encoded_inputs.items():
                if k == "invars" or k == "t_start" or k == "seq_index":
                    continue
                seq_ = (
    ...
    def pad(
        ...
        # The model's main input name, usually `time_seqs`, has to be passed for padding
        ...
        if max_length is None:
            if padding_strategy == PaddingStrategy.LONGEST:
                max_length_arg = max(len(inputs) for inputs in required_input)
                padding_strategy = PaddingStrategy.MAX_LENGTH
    ...
    def _pad(
        ...
        needs_to_be_padded = (
            padding_strategy != PaddingStrategy.DO_NOT_PAD
            and ~is_all_seq_equal_max_length
        )
        ...
        seq_pad_mask = np.full_like(
            batch_output[self.model_input_names[2]], fill_value=True, dtype=bool
        )
        for i, seq_len in enumerate(seq_lens):
            if self.padding_side == "right":
                seq_pad_mask[i, seq_len:] = False
            else:
                seq_pad_mask[i, : (max_length - seq_len)] = False
        ...
        if return_attention_mask:
            ...
            batch_output[self.model_input_names[4]] = np.array([])

        # t_start
        if self.model_input_names[5] in encoded_inputs:
            batch_output[self.model_input_names[5]] = encoded_inputs[
                self.model_input_names[5]
            ]
        else:
            batch_output[self.model_input_names[5]] = np.array([float("NaN")])

        # No need to pad invariants
        if self.model_input_names[6] in encoded_inputs and (
            self.use_invariant_features is True
        ):
            batch_output[self.model_input_names[6]] = encoded_inputs[
                self.model_input_names[6]
            ]
        else:
            batch_output[self.model_input_names[6]] = np.array([])

        # seq_index
        if self.model_input_names[7] in encoded_inputs:
            batch_output[self.model_input_names[7]] = encoded_inputs[
                self.model_input_names[7]
            ]
        else:
            batch_output[self.model_input_names[7]] = np.array([float("NaN")])
    ...
    def make_pad_sequence(
        ...
        group_by_event_types: bool = False,
    ) -> np.ndarray:
        ...
        Args:
            ...
            max_len (int): the maximum length of the sequence after padding.
        ...
        if not group_by_event_types:
            ...
            pad_seq_list: list[np.ndarray] = []
            for _seq in seqs:
                if padding_side == "right":
                    pad_seq_list.append(
                        np.array(
                            [s + [pad_token_id] * (max_len - len(s)) for s in seqs],
                            dtype=dtype,
                        )
                    )
                else:
            ...
            pad_seq = np.array(pad_seq_list)
        return pad_seq

    def make_attn_mask_for_pad_sequence(
        self, pad_seqs: np.ndarray, pad_token_id: int
    ) -> np.ndarray:
        """Make the attention masks for the sequence.

        Args:
            pad_seqs (array): list of sequences that have been padded with fixed length
    ...
    def make_type_mask_for_pad_sequence(self, pad_seqs: np.ndarray) -> np.ndarray:
        """Make the type mask.

        Args:
            pad_seqs (array): a list of sequence events with equal length (i.e., padded sequence)
